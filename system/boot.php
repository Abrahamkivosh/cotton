...
//    $rrr = ORM::for_table('tbl_rights_group')->where('name',$_SESSION['user_type'])->find_one();
  //  if($rrr) {
       // $close_to = ORM::for_table('tbl_roles')->where('uid',$rrr->id)->find_many();
       $close_to = ORM::for_table('tbl_roles')->find_many();
        $rights=array();
        foreach ($close_to as $close) {
            $rii = ORM::for_table('tbl_rights')->where('id',$close['rid'])->find_one();
            $rights[$rii->name] = 1;
        }
        $ui->assign('rights', $rights);
