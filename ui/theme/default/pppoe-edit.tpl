	...
	<div class="card-body">
                <form class="form-horizontal" method="post" role="form" action="{$_url}services/edit-pppoe-post">
							...
							<div class="form-group">
								...
								<div class="form-group">
								<label class="col-md-2 control-label">Burst {$_L['BW_Name']}</label>
								<div class="col-md-6">
									<select id="personSelect" id="id_bw" name="id_bw_b" class="form-control">
										{foreach $b as $bs}
											<option value="{$bs['id']}" {if $d['id_bw'] eq $bs['id']} selected {/if}>{$bs['name_bw']}</option>
										{/foreach}
									</select>
							...
							<div class="form-group">
								...
								<div class="col-md-6">
									<select id="personSelect"  name="bridge" class="form-control" >
										 <option value="{$d['bridge']}">{$d['bridge']}</option>
										 {$num =count($ARRAY)}
										{for $i=0; $i<$num; $i++}
							...
							<div class="form-group">
								...
								<div class="col-md-6">
									<select id="personSelect" id="pool_name" name="pool_name" class="form-control">
										{foreach $p as $ps}
